...
```
broxeen/
    ├── config        ├── d        ├── spec        ├── spec        ├── spec    ├── config        ├── spec        ├── spec        ├── spec        ├── spec        ├── spec        ├── types        ├── deviceRepository        ├── historyRepository        ├── scanHistoryRepository        ├── migrations        ├── databaseManager        ├── configuredDeviceRepository        ├── chatRepository        ├── copyContextCommand        ├── sendMessageCommand        ├── saveSettingsCommand        ├── browseCommand        ├── sttClient        ├── llmPrompts        ├── logger        ├── resolver        ├── llmClient        ├── phonetic        ├── runtime        ├── useChatPersistence        ├── useTts        ├── useDatabaseManager        ├── useChatDispatch        ├── useHistoryPersistence        ├── browseGateway        ├── useChatSpeech        ├── useAlertBridge        ├── useLlm        ├── useWatchNotifications        ├── useSpeech        ├── useChatMessages        ├── llmService        ├── defaultBrowseAdapter        ├── defaultLlmAdapter        ├── browseService        ├── useStt        ├── types        ├── preferenceLearning            ├── types        ├── queryBus        ├── pluginRegistry        ├── realtimeSync        ├── cache        ├── bootstrap        ├── intentRouter        ├── processRegistry        ├── systemContext        ├── retry        ├── actionSchema        ├── intentSchema        ├── commandBus        ├── fallbackHandler        ├── types        ├── llmIntentClassifier        ├── networkScanner    ├── motion_pipeline        ├── serviceProber        ├── getMessagesQuery        ├── getSettingsQuery        ├── monitoringPlugin        ├── appConfig        ├── watchConfig        ├── autoConfig        ├── audioSettings        ├── chatAggregate        ├── chatEvents        ├── configStore        ├── eventStore        ├── quickActionResolver        ├── errorReporting        ├── types        ├── healthCheck        ├── alertBridge        ├── autoWatchIntegration        ├── changeDetector        ├── watchManager            ├── wakeOnLanPlugin            ├── remoteMachinePlugin            ├── onvifPlugin            ├── pingPlugin            ├── mdnsPlugin            ├── arpPlugin            ├── portScanPlugin├── resolver            ├── processesPlugin                ├── index            ├── dockerPlugin            ├── fileSearchPlugin            ├── diskInfoPlugin        ├── monitor/            ├── logsPlugin            ├── sshPlugin            ├── advancedPortScanPlugin            ├── motionDetectionPlugin            ├── serviceProbePlugin            ├── cameraDetection            ├── deviceStatusPlugin            ├── autoScanScheduler            ├── rtspCameraPlugin        ├── rtsp-camera/        ├── marketplace/            ├── deviceConfigPlugin            ├── marketplaceLoader        ├── cameras/            ├── cameraHealthPlugin            ├── cameraPtzPlugin        ├── mqtt/            ├── cameraSnapshotPlugin            ├── mqttPlugin            ├── scopeRegistry            ├── browsePlugin            ├── authBrowsePlugin        ├── voice/            ├── frigateEventsPlugin            ├── voiceCommandsPlugin            ├── chatPlugin            ├── cameraVendorDatabase            ├── toonicBridgePlugin            ├── emailPlugin        ├── protocol-bridge/            ├── cameraLivePlugin    ├── main    ├── App        ├── WatchBadge        ├── NetworkSelector        ├── HealthDiagnostic            ├── networkScanPlugin        ├── AlertBridgeComponent        ├── ChatOverlays        ├── Settings        ├── CameraPreview        ├── NetworkHistorySelector        ├── CameraLiveInline        ├── SetupWizardModal        ├── MessageResultCard        ├── MicSettingsModal        ├── QuickActionButtons            ├── protocolBridgePlugin            ├── simple        ├── TtsControls        ├── QuickCommands        ├── Chat        ├── DeviceDashboardModal        ├── MessageQuickActions        ├── CommandHistory        ├── ActionSuggestions        ├── DiagnosticsModal        ├── ThinkingMessage        ├── ChatPersistenceBridge        ├── ChatInput        ├── ChatConfigPrompt        ├── FileResultsDisplay        ├── TtsSettingsModal        ├── DeviceStrip        ├── ChatMessageList        ├── ErrorReportPanel        ├── QuickCommandHistory    ├── config    ├── config        ├── CqrsContext    ├── add-reolink-camera        ├── pluginContext        ├── network_info    ├── net-diag        ├── vision_config        ├── main_changes        ├── vision_scene_buffer        ├── vision_tracker        ├── llm            ├── monitorPlugin        ├── vision_motion        ├── tts        ├── frigate_mqtt        ├── llm_query        ├── stt        ├── browse_rendered        ├── file_search        ├── vision_llm        ├── main        ├── rss_parser        ├── vision_movement        ├── logging        ├── query_schema        ├── autostart        ├── motion_detection        ├── vision_detector        ├── vision_pipeline        ├── ssh        ├── email        ├── vision_db        ├── disk_info        ├── audio_commands        ├── vision_query_engine        ├── docker        ├── vision_capture        ├── remote_machine        ├── wake_word        ├── toonic_sidecar        ├── settings        ├── audio_capture        ├── network_scan        ├── local_llm        ├── network    ├── chat-cli        ├── content_cleaning        ├── file_search_bench├── project    ├── install-openvino-rhel    ├── detect-platform    ├── install-openvino-pip    ├── install-openvino    ├── install-openvino-docker    ├── install-openvino-macos        ├── setup-audio    ├── install-openvino-ubuntu        ├── tts_backend        ├── content_extraction```
