...
![version](https://img.shields.io/badge/version-0.1.0-blue) ![python](https://img.shields.io/badge/python-%3E%3D3.9-blue) ![coverage](https://img.shields.io/badge/coverage-unknown-lightgrey) ![functions](https://img.shields.io/badge/functions-3704-green)
> **3704** functions | **444** classes | **229** files | CC̄ = 4.1
...
```
broxeen/
    ├── config    ├── config        ├── d        ├── spec        ├── spec        ├── spec        ├── spec        ├── spec        ├── spec        ├── spec        ├── spec        ├── deviceRepository        ├── types        ├── historyRepository        ├── scanHistoryRepository        ├── migrations        ├── databaseManager        ├── configuredDeviceRepository        ├── chatRepository        ├── copyContextCommand        ├── sendMessageCommand        ├── saveSettingsCommand        ├── browseCommand        ├── browseGateway        ├── sttClient        ├── llmPrompts        ├── logger        ├── resolver├── resolver        ├── llmClient        ├── runtime        ├── phonetic    ├── motion_pipeline        ├── useChatPersistence        ├── useDatabaseManager        ├── useHistoryPersistence├── sitecustomize        ├── useTts        ├── useAlertBridge        ├── useChatSpeech        ├── useWatchNotifications        ├── useLlm        ├── useChatMessages        ├── llmService        ├── defaultBrowseAdapter        ├── useStt        ├── defaultLlmAdapter        ├── browseService        ├── types            ├── types        ├── useSpeech        ├── preferenceLearning        ├── useChatDispatch        ├── queryBus        ├── pluginRegistry        ├── realtimeSync        ├── systemContext        ├── cache        ├── processRegistry        ├── retry        ├── bootstrap        ├── intentRouter        ├── intentSchema        ├── commandBus        ├── types        ├── fallbackHandler        ├── llmIntentClassifier        ├── actionSchema        ├── getSettingsQuery        ├── getMessagesQuery        ├── serviceProber        ├── monitoringPlugin        ├── appConfig        ├── watchConfig        ├── autoConfig        ├── configStore        ├── audioSettings        ├── chatAggregate        ├── chatEvents        ├── networkScanner        ├── eventStore        ├── errorReporting        ├── types        ├── quickActionResolver        ├── autoWatchIntegration        ├── alertBridge        ├── healthCheck        ├── changeDetector            ├── onvifPlugin        ├── watchManager            ├── remoteMachinePlugin            ├── wakeOnLanPlugin            ├── mdnsPlugin            ├── pingPlugin            ├── portScanPlugin            ├── arpPlugin                ├── index            ├── processesPlugin            ├── diskInfoPlugin            ├── fileSearchPlugin            ├── dockerPlugin        ├── monitor/            ├── logsPlugin            ├── sshPlugin            ├── serviceProbePlugin            ├── advancedPortScanPlugin            ├── motionDetectionPlugin            ├── cameraDetection            ├── deviceStatusPlugin            ├── autoScanScheduler            ├── rtspCameraPlugin        ├── rtsp-camera/        ├── marketplace/            ├── marketplaceLoader            ├── deviceConfigPlugin            ├── cameraPtzPlugin        ├── cameras/            ├── cameraHealthPlugin        ├── mqtt/            ├── cameraSnapshotPlugin            ├── mqttPlugin            ├── browsePlugin            ├── scopeRegistry            ├── authBrowsePlugin        ├── voice/            ├── frigateEventsPlugin            ├── voiceCommandsPlugin            ├── chatPlugin            ├── cameraVendorDatabase            ├── toonicBridgePlugin            ├── networkScanPlugin        ├── protocol-bridge/            ├── emailPlugin    ├── main            ├── cameraLivePlugin        ├── WatchBadge    ├── App        ├── NetworkSelector        ├── HealthDiagnostic        ├── AlertBridgeComponent        ├── ChatOverlays        ├── Settings        ├── NetworkHistorySelector        ├── CameraPreview        ├── SetupWizardModal        ├── MessageResultCard        ├── CameraLiveInline        ├── MicSettingsModal        ├── QuickActionButtons            ├── protocolBridgePlugin            ├── simple        ├── TtsControls            ├── monitorPlugin        ├── QuickCommands        ├── Chat        ├── DeviceDashboardModal        ├── CommandHistory        ├── MessageQuickActions        ├── ActionSuggestions        ├── DiagnosticsModal        ├── ChatPersistenceBridge        ├── ThinkingMessage        ├── ChatConfigPrompt        ├── TtsSettingsModal        ├── ChatInput        ├── FileResultsDisplay        ├── DeviceStrip        ├── QuickCommandHistory        ├── ErrorReportPanel    ├── config    ├── config        ├── CqrsContext        ├── pluginContext    ├── add-reolink-camera        ├── ChatMessageList        ├── network_info        ├── main_changes        ├── vision_tracker    ├── net-diag        ├── vision_config        ├── llm        ├── vision_scene_buffer        ├── tts        ├── vision_motion        ├── frigate_mqtt        ├── llm_query        ├── stt        ├── file_search        ├── browse_rendered        ├── vision_llm        ├── rss_parser        ├── vision_movement        ├── main        ├── query_schema        ├── logging        ├── autostart        ├── motion_detection        ├── vision_pipeline        ├── vision_detector        ├── ssh        ├── email        ├── audio_commands        ├── vision_db        ├── disk_info        ├── vision_query_engine        ├── docker        ├── vision_capture        ├── remote_machine        ├── wake_word        ├── settings        ├── toonic_sidecar        ├── network_scan        ├── network        ├── local_llm        ├── audio_capture        ├── content_cleaning        ├── file_search_bench├── project    ├── install-openvino-rhel    ├── detect-platform    ├── install-openvino-pip    ├── install-openvino    ├── install-openvino-docker    ├── install-openvino-ubuntu    ├── install-openvino-macos        ├── setup-audio    ├── chat-cli        ├── content_extraction        ├── tts_backend```
